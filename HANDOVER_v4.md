...
11. **Single-viewport verify goal** - The verify step should aim to show controls, visuals, and results simultaneously without scrolling. For complex questions this may require compact side-by-side layouts. Flag for future: consider collapsible sections or tabbed views.
...
29. **Minimum readable sizes - THIS IS NON-NEGOTIABLE** - Every diagram, graph, number line, or SVG visualisation must be large enough to read comfortably without squinting. Minimum plot area for ANY diagram: pW >= 320, pH >= 180. Minimum SVG font size: 10px. Minimum axis/grid label font: 9px. For the Verify step, diagrams should be FULL WIDTH (pW >= 450, pH >= 250) since this is the main interactive element. If a split-pane layout makes either panel too small, use full-width stacked layout instead. A large, clear, readable diagram is ALWAYS better than a cramped side-by-side layout. When in doubt, make it bigger.
...
33. **Verify layout is flexible, not templated** - The Verify step does NOT have to follow one fixed layout. Choose the layout that best serves the specific question's visuals, but ALL content must fill the same container width as the rest of the walkthrough (maxWidth ~820px). Options include: full-width diagram with controls below; two full-width diagrams stacked vertically (e.g. circle in xy-plane on top, number line below, both spanning the full container); side-by-side ONLY when both panels are large enough to read comfortably (each at least 380px). The Verify Design Principles table specifies WHAT interactive element to use, not HOW to arrange it on screen. Let the content dictate the layout. If a question has a large geometric diagram (circle, triangle, polygon), give it full container width and put sliders/status cards below. If a question has two complementary views (e.g. number line + circle diagram), stack them vertically at full container width rather than squeezing them side by side at half width.
